...
// Process structure to hold each process's data
typedef struct {
    int pid;        // Process ID
...
// Swap function used if needed (not used directly here)
...
void fcfs(Process proc[], int n) {
    qsort(proc, n, sizeof(Process), compareArrival); // Sort by arrival time
    ...
    for (int i = 0; i < n; i++) {
        ...
        current_time += proc[i].burst; // Move time forward by burst
    }
...
void sjf(Process proc[], int n) {
    ...
    while (completed < n) {
        ...
        if (idx == -1) {
            current_time++; // If no process is ready, wait 1 unit
            continue;
        }
...
void roundRobin(Process proc[], int n, int quantum) {
    ...
    int *queue = malloc(n * 10 * sizeof(int));
    int front = 0, rear = 0;
